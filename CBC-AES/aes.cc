...
void Aes::addRoundKey(std::vector<std::vector<std::bitset<8> > > mtx) {
    ...
    int lineslt = 0;
    std::cout << "Esta es la llamada " << countercbc<< std::endl;
...
std::vector<std::bitset<8> > Aes::toCbc() {
    std::vector<std::vector<std::bitset<8> > > transpose;
    std::vector<std::bitset<8> > result;
    ...
    for(int i = 0; i < rowsize; ++i)
            for(int j=0; j < 4; ++j) {
                transpose[j][i] = intermediate_[i][j];
        }
    for(int i = 0; i < rowsize; ++i)
            for(int j=0; j < 4; ++j) {
                result.push_back(transpose[i][j]);
        }
    return result;
}
